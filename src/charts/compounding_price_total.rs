...
pub fn generate(trades: &mut HashMap<Period, Vec<Order>>, _periods: &PeriodConfig, grid: &Grid) {
    ...
    std::fs::write(
        "charts/compounding-price-total-ask.svg",
    ...
    std::fs::write(
        "charts/compounding-price-total-bar-bid.svg",
        bar_chart_bid.svg().unwrap(),
    )
    .unwrap();
    let bar_chart_ask = bar_chart(trades, grid, OrderSide::Ask);
    std::fs::write(
        "charts/compounding-price-total-bar-ask.svg",
        bar_chart_ask.svg().unwrap(),
    )
    .unwrap();
}
    ...
    let mut line_chart = LineChart::new_with_theme(
        vec![
            (
                ...
                compounding_prices(trades, order_side),
            )
                .into(),
            (
                format!("Average {} Prices with Grid", order_side).as_str(),
                compounding_prices_no_market(trades, order_side, grid),
            )
                .into(),
        ],
        (1..=12).map(|period| format!("{}", period)).collect(),
        THEME_GRAFANA,
    );
...
fn bar_chart(
    ...
    let series_data = vec![
        year_gain(trades, order_side),
        year_gain_no_market(trades, order_side, grid),
    ];
...
fn compounding_prices(trades: &HashMap<Period, Vec<Order>>, order_side: OrderSide) -> Vec<f32> {
    ...
    utils::average_prices(&total_period_prices, 31)
        .iter()
        .map(|p| *p / 1000.0)
        .collect::<Vec<f32>>()
}
