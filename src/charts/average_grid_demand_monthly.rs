...
pub fn generate(trades: &HashMap<Period, Vec<Order>>, periods: &PeriodConfig, grid: &Grid) {
    ...
    std::fs::write("charts/average-grid-demand-monthly.svg", line_chart.svg().unwrap()).unwrap();
}

/// Returns (with_p2p, without_p2p) average grid demand per month (0-11).
fn average_grid_demand_monthly(trades: &HashMap<Period, Vec<Order>>, periods: &PeriodConfig, _grid: &Grid) -> (Vec<f32>, Vec<f32>) {
